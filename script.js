document.addEventListener('DOMContentLoaded', () => {
    ...
    createPostitBtn.addEventListener('click', () => {
        ...
        // Adiciona os Handles de Conexão (Pontos para iniciar/terminar linha)
        const topHandle = createHandle(postId, 'top');
        const bottomHandle = createHandle(postId, 'bottom');
        postit.appendChild(topHandle);
        postit.appendChild(bottomHandle);
    ...
    function removePostitAndConnections(postId) {
        ...
        const linesToRemove = connections.filter(c => c.sourceId === postId || c.targetId === postId);
        linesToRemove.forEach(conn => conn.svgElement.remove());

        // Atualiza o array de conexões
        connections.splice(0, connections.length, 
            ...connections.filter(c => c.sourceId !== postId && c.targetId !== postId)
        );
    }
    ...
    function updateAllLines() {
        connections.forEach(conn => {
            ...
            if (sourcePostit && targetPostit) {
                ...
                const sourceHandle = sourcePostit.querySelector('.connect-handle.bottom') || sourcePostit.querySelector('.connect-handle.top');
                const targetHandle = targetPostit.querySelector('.connect-handle.top') || targetPostit.querySelector('.connect-handle.bottom');
    ...
    postitArea.addEventListener('mousedown', (e) => {
        ...
        if (postit && !e.target.closest('.close-postit') && !e.target.closest('.connect-handle')) {
            activePostit = postit;
            initialX = e.clientX;
            initialY = e.clientY;
